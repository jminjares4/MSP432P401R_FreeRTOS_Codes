...
COORDINATE setCoordinates(const NMEA_GNRMC *data){
    ...
    sprintf(tempCoordinates.latitude, "%lf",latitudeDegrees(data->latitude, data->latitudeDirection));
    sprintf(tempCoordinates.longitude,"%lf",longitudeDegrees(data->longitude, data->longitudeDirection));
...
double latitudeDegrees(char *latitude, char *direction){
  double latitudeAsDouble = strtod(latitude,NULL);
...
double longitudeDegrees(char *longitude, char *direction){
  double longitudeAsDouble = strtod(longitude,NULL);
